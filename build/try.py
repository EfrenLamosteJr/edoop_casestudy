...
from auth import (create_staff_account, send_rejection_email, send_approval_email, approve_verification_request,
                  reject_verification_request,
                  send_finish_email, update_request_status, delete_request, get_finished_requests,
                  get_request_details_with_resident, get_pending_requests, delete_staff,
                  delete_resident, send_deletion_email)
...
# --- ginawa ko ng global yung logo nag eerror e ---
...
def show_request_management_content(parent_frame):
    ...
    if not pending_requests:
        ...
        for request in pending_requests:
            ...
            process_btn = ctk.CTkButton(action_buttons_frame, text="Process", width=60, height=25,
                                        fg_color=APPROVE_COLOR, font=ctk.CTkFont(size=10))
            ...
            def view_action(req_id):
                ...
                ctk.CTkButton(button_frame, text="Close", command=popup.destroy, fg_color=VIEW_COLOR,
                              hover_color=SIDEBAR_BTN_HOVER).grid(row=0, column=0, padx=10, pady=10, sticky="ew")
    ...
    finished_title = ctk.CTkLabel(scrollable_frame, text=finished_title_text, font=ctk.CTkFont(size=18, weight="bold"),
                                  anchor="w")
    ...
    if not finished_requests:
        ctk.CTkLabel(finished_container, text="No finished requests.", text_color="gray",
        ...
        ctk.CTkLabel(header_frame_finished, text="ID", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0,
                                                                                             sticky="w", padx=5)
        ctk.CTkLabel(header_frame_finished, text="RESIDENT NAME", font=ctk.CTkFont(weight="bold")).grid(row=0, column=1,
                                                                                                        sticky="w",
                                                                                                        padx=5)
        ctk.CTkLabel(header_frame_finished, text="DOCUMENT REQUESTED", font=ctk.CTkFont(weight="bold")).grid(row=0,
                                                                                                             column=2,
                                                                                                             sticky="w",
                                                                                                             padx=5)
        ctk.CTkLabel(header_frame_finished, text="STATUS", font=ctk.CTkFont(weight="bold")).grid(row=0, column=3,
                                                                                                 sticky="e", padx=5)
        ...
        for request in finished_requests:
            ...
            ctk.CTkLabel(row_frame, text=request['full_name']).grid(row=0, column=1, sticky="w", padx=5)
            ctk.CTkLabel(row_frame, text=request['document_name']).grid(row=0, column=2, sticky="w", padx=5)
            ctk.CTkLabel(row_frame, text="Finished", font=ctk.CTkFont(size=10, weight="bold"), fg_color=APPROVE_COLOR,
                         text_color="white", corner_radius=5, width=60).grid(row=0, column=3, sticky="e", padx=5)
...
def show_dashboard_content(parent_frame, navigate_callback):
    ...
    create_stat_card(stats_frame, "New Registrations (Pending)", pending_verify_account, "➕", "#E74C3C").grid(row=0,
                                                                                                              column=0)
    ...
    create_action_button(actions_frame, "✅  Approve Residents", "#2ECC71",
                         command=lambda: navigate_callback("residents")).grid(row=0, column=0)
    create_action_button(actions_frame, "▶️  Process Requests", "#3498DB",
                         command=lambda: navigate_callback("requests")).grid(row=0, column=1)
    create_action_button(actions_frame, "👁️  View All Residents", "#3498DB",
                         command=lambda: navigate_callback("residents")).grid(row=0, column=2)
    create_action_button(actions_frame, "⚙️  Manage Staff", "#95A5A6", command=lambda: navigate_callback("staff")).grid(
        row=0, column=3)
...
def show_resident_accounts_content(parent_frame):
    ...
    if not pending_residents:
        ...
        for resident in pending_residents:
            ...
            ctk.CTkButton(action_buttons_frame, text="Approve", width=60, height=25, fg_color=APPROVE_COLOR,
                          font=ctk.CTkFont(size=10),
                          command=lambda id=resident['id'], email=resident['email']: approve_action(id, email)).pack(
                side="left", padx=2)
    ...
    if not approved_residents:
        ...
        header_frame_approved.grid_columnconfigure((1, 2), weight=1)  # Adjusted for 4 columns: ID, NAME, EMAIL, ACTION
        header_frame_approved.grid_columnconfigure(3, weight=0)
        ...
        ctk.CTkLabel(header_frame_approved, text="EMAIL", font=ctk.CTkFont(weight="bold")).grid(row=0, column=2,
                                                                                                sticky="w", padx=5)
        ctk.CTkLabel(header_frame_approved, text="ACTION", font=ctk.CTkFont(weight="bold")).grid(row=0, column=3,
                                                                                                 sticky="e", padx=5)

        for resident in approved_residents:
            row_frame = ctk.CTkFrame(approved_container, fg_color="transparent")
            row_frame.pack(fill="x", padx=10, pady=5)
            row_frame.grid_columnconfigure((1, 2), weight=1)  # Adjusted for 4 columns
            row_frame.grid_columnconfigure(3, weight=0)
            ...
            # ACTION buttons frame for verified residents
            action_buttons_frame = ctk.CTkFrame(row_frame, fg_color="transparent")
            action_buttons_frame.grid(row=0, column=3, sticky="e")
            ...
            def delete_resident_action(res_id, name, email):
                # Deletion popup with reason textbox
                delete_popup = ctk.CTkToplevel(parent_frame)
                delete_popup.title("Delete Resident Account")
                delete_popup.geometry("400x300")
                delete_popup.transient(parent_frame)
                delete_popup.grab_set()

                ctk.CTkLabel(delete_popup, text="Reason for Deletion:", font=ctk.CTkFont(size=14)).pack(pady=(20, 10))
                reason_text = ctk.CTkTextbox(delete_popup, height=100)
                reason_text.pack(pady=(0, 20), padx=20, fill="x")

                def send_delete():
                    reason = reason_text.get("1.0", "end").strip()
                    if not reason:
                        ctk.CTkLabel(delete_popup, text="Please enter a reason.", text_color="red").pack(pady=10)
                        return
                    if delete_resident(res_id) and send_deletion_email(email, reason):
                        print(f"Deleted resident {name} and emailed {email}")
                        refresh_content()
                    else:
                        print(f"Failed to delete resident {name}")
                    delete_popup.destroy()

                ctk.CTkButton(delete_popup, text="Delete Account", command=send_delete, fg_color=REJECT_COLOR).pack(
                    pady=10)

            ctk.CTkButton(action_buttons_frame, text="Delete", width=60, height=25, fg_color=REJECT_COLOR,
                          font=ctk.CTkFont(size=10),
                          command=lambda id=resident['id'], name=resident['name'],
                                         email=resident['email']: delete_resident_action(id, name, email)).pack(
                side="left", padx=2)
...
def show_staff_accounts_content(parent_frame):
    ...
    ctk.CTkLabel(create_frame, text="Create New Staff Account", font=ctk.CTkFont(size=18, weight="bold"),
    ...
    create_button = ctk.CTkButton(form_grid, text="+ Create Account", height=40, font=ctk.CTkFont(weight="bold"),
    ...
    existing_title = ctk.CTkLabel(scrollable_frame, text=existing_title_text, font=ctk.CTkFont(size=18, weight="bold"),
                                  anchor="w")
    ...
    if not existing_staff:
        ctk.CTkLabel(existing_container, text="No existing staff accounts found.", text_color="gray",
                     font=ctk.CTkFont(size=12)).pack(pady=20)
    else:
        ...
        header_frame_existing.grid_columnconfigure((1, 2, 3), weight=1)  # Adjusted for 5 columns now
        header_frame_existing.grid_columnconfigure((0, 4), weight=0)

        # Updated headers: Removed "STATUS"
        headers = ["ID", "NAME", "USERNAME", "POSITION", "ROLE", "ACTION"]
        for i, header in enumerate(headers):
            sticky_val = "w" if i < len(headers) - 1 else "e"
            ctk.CTkLabel(header_frame_existing, text=header, font=ctk.CTkFont(weight="bold", size=11)).grid(row=0,
                                                                                                            column=i,
                                                                                                            sticky=sticky_val,
                                                                                                            padx=5)
        ...
        for staff in existing_staff:
            ...
            row_frame.grid_columnconfigure((1, 2, 3), weight=1)  # Adjusted for 5 columns
            row_frame.grid_columnconfigure((0, 4), weight=0)

            ctk.CTkLabel(row_frame, text=f"#{staff['id']}", font=ctk.CTkFont(size=11)).grid(row=0, column=0, sticky="w",
                                                                                            padx=5)
            ctk.CTkLabel(row_frame, text=staff['name'], font=ctk.CTkFont(size=11), anchor="w").grid(row=0, column=1,
                                                                                                    sticky="ew", padx=5)
            ctk.CTkLabel(row_frame, text=staff['username'], font=ctk.CTkFont(size=11), anchor="w").grid(row=0, column=2,
                                                                                                        sticky="ew",
                                                                                                        padx=5)
            ctk.CTkLabel(row_frame, text=staff['position'] or "-", font=ctk.CTkFont(size=11), anchor="w").grid(row=0,
                                                                                                               column=3,
                                                                                                               sticky="ew",
                                                                                                               padx=5)

            # Role label (kept as is)
            role_label = ctk.CTkLabel(row_frame, text=staff['role'], font=ctk.CTkFont(size=10, weight="bold"),
            ...
            # Action buttons: Removed "View", added "Delete"
            action_buttons_frame = ctk.CTkFrame(row_frame, fg_color="transparent")
            action_buttons_frame.grid(row=0, column=5, sticky="e")  # Adjusted column index

            def delete_staff_action(staff_id, name):
                # Confirmation popup
                confirm_popup = ctk.CTkToplevel(parent_frame)
                confirm_popup.title("Confirm Deletion")
                confirm_popup.geometry("300x150")
                confirm_popup.transient(parent_frame)
                confirm_popup.grab_set()
                ctk.CTkLabel(confirm_popup, text=f"Are you sure you want to delete\n{name}?",
                ...
                def confirm_delete():
                    if delete_staff(staff_id):
                        ...
                        for widget in parent_frame.winfo_children():
                            widget.destroy()
                        show_staff_accounts_content(parent_frame)
                    else:
                        print(f"Failed to delete staff {name}")
                    confirm_popup.destroy()

                ctk.CTkButton(confirm_popup, text="Yes", fg_color=REJECT_COLOR, command=confirm_delete).pack(
                    side="left", padx=20, pady=10)
                ctk.CTkButton(confirm_popup, text="No", command=confirm_popup.destroy).pack(side="right", padx=20,
                                                                                            pady=10)

            ctk.CTkButton(action_buttons_frame, text="Delete", width=60, height=25, fg_color=REJECT_COLOR,
                          font=ctk.CTkFont(size=10),
                          command=lambda id=staff['id'], name=staff['name']: delete_staff_action(id, name)).pack(
                side="left", padx=2)
...
def show_placeholder_content(parent_frame, page_name):
    ...
    ctk.CTkLabel(parent_frame, text=f"Content for {page_name} goes here.", font=ctk.CTkFont(size=18),
...
def start_admin_dashboard():
    ...
    content_container = ctk.CTkFrame(root, fg_color=CONTENT_BG, corner_radius=0);
    content_container.grid(row=0, column=1, sticky="nsew", padx=20, pady=20);
    content_container.grid_rowconfigure(1, weight=1);
    content_container.grid_columnconfigure(0, weight=1)
    main_header_label = ctk.CTkLabel(content_container, text="", font=ctk.CTkFont(size=24, weight="bold"), anchor="w",
                                     text_color=TEXT_COLOR_DARK);
    main_header_label.grid(row=0, column=0, sticky="ew", pady=(0, 20))
    dynamic_content_frame = ctk.CTkFrame(content_container, fg_color="transparent");
    dynamic_content_frame.grid(row=1, column=0, sticky="nsew")
    ...
    def update_sidebar_selection(active_key):
        ...
        for key, button_widget in button_widgets.items():
            ...
            button_widget.configure(fg_color=ACTIVE_BTN_BG if is_active else "transparent",
    ...
    def navigate_to(page_key):
        ...
        if page_key == "dashboard":
            show_dashboard_content(dynamic_content_frame, navigate_to)
        elif page_key == "requests":
            show_request_management_content(dynamic_content_frame)
        elif page_key == "residents":
            show_resident_accounts_content(dynamic_content_frame)
        elif page_key == "staff":
            show_staff_accounts_content(dynamic_content_frame)
        else:
            show_placeholder_content(dynamic_content_frame, "Unknown Page")

    # --- Sidebar Widgets ---
    # (Sidebar creation remains the same - collapsed)
    sidebar_title = ctk.CTkLabel(sidebar_frame, text="P2SERVE Admin Panel", font=ctk.CTkFont(size=20, weight="bold"),
                                 text_color="white");
    sidebar_title.pack(pady=20, padx=20, anchor="w")
    for key, (icon, text) in nav_buttons.items():
        button = ctk.CTkButton(sidebar_frame, text=f" {icon}  {text}", fg_color="transparent", text_color="white",
                               hover_color=SIDEBAR_BTN_HOVER, font=ctk.CTkFont(size=14), anchor="w", height=40,
                               corner_radius=8, command=lambda k=key: navigate_to(k));
    ...
    def logout_action():
        ...
        root.destroy()
        try:
            from Admin_Log_In_GUI import start_admin_login; start_admin_login()
        except ImportError:
            print("Error: Could not import Admin_Log_In_GUI.py")

    logout_btn = ctk.CTkButton(sidebar_frame, text="➡️  Log Out", fg_color="transparent", text_color="white",
                               hover_color=SIDEBAR_BTN_HOVER, font=ctk.CTkFont(size=14), anchor="w", height=40,
                               corner_radius=8, command=logout_action);
